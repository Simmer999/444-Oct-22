...
function init() {
    ...
    camera = new THREE.PerspectiveCamera( 40, window.innerWidth / window.innerHeight, 1, 1500 );//Affects the size of the letters in the opening of the page.
    camera.position.set( 0, 400, 700 );
    cameraTarget = new THREE.Vector3( 0, 150, 0 );
    scene = new THREE.Scene();
    scene.background = new THREE.Color( 0x000000 );//Setting the background colour.
    ...
    const plane = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial( { color: 0xffffff, side: THREE.DoubleSide, opacity: 0.5, transparent: true } )//DoubleSide the plane.
    );
...
function loadFont() {
    ...
    loader.load( 'https://unpkg.com/three@0.139/examples/fonts/' + fontName + '_' + fontWeight + '.typeface.json', function ( response ) {
        font = response;
        refreshText();
    } );
}
